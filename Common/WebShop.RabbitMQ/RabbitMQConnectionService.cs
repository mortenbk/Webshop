...
{
    public class RabbitMQConnectionService : IEventBus
    {

        const string BROKER_NAME = "webshop-rabbit";
        ...
        private readonly string AUTOFAC_SCOPE_NAME = "webshop-rabbit";
        ...
        public void Publish(MessageQueueEvent @event)
        {
            if (!_connection.IsConnected)
            {
                _connection.TryConnect();
            ...
            if (_subsManager.HasSubscriptionsForEvent(eventName))
            {
                using (var scope = _autofac.BeginLifetimeScope(AUTOFAC_SCOPE_NAME))
                {
                    ...
                    foreach (var subscription in subscriptions)
                    {
                        ...
                        var concreteType = typeof(IMessageQueueEventHandler<>).MakeGenericType(eventType);

                        await Task.Yield();
                        await (Task)concreteType.GetMethod("HandleMessage").Invoke(handler, new object[] { integrationEvent });
